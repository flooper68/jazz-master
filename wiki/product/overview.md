...
storage clearing, or browser moves. ADR-012 changes the destination: Clerk will
own identity, `/app/*` will require sign-in, browser code will talk to typed
tRPC procedures, server code will own Drizzle/Postgres access, and Postgres will
become the source of truth for long-run app data. Existing local browser data is
...
shipped as TASK-021): Astro serves a barebones landing page at `/` and hosts the
...
`/app/history`, …). The build targets Cloudflare Workers; a typed tRPC API
surface now exists (TASK-023/TASK-064 — scaffolding only: `/trpc/health`,
server-only `/trpc/dbSmoke`, and a mock-only `mockPractice.record` write/read
path; no real product practice feature uses the server yet). Since
TASK-055/TASK-061, server-side Postgres work has Drizzle migration
