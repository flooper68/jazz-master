...
import { resetTrpcTestData, seedTrpcTestRuns, trpcTestFetch } from '../test/trpcTestFetch'
...
describe('the library tools in the browser', () => {
  ...
  it('tell arguments the server will not take, and a signed-out user, from a lost connection', async () => {
    ...
    expect(await setUp({ client: answering('BAD_REQUEST', 400) }).call('list_exercises')).toMatchObject({ status: 'invalid' })
    expect(await setUp({ client: answering('UNAUTHORIZED', 401) }).call('list_exercises')).toMatchObject({ status: 'error', message: expect.stringContaining('signed out') })
  })
  ...
  it('turn a failed request into an answer an agent can read', async () => {
    ...
    expect(await setUp({ client: offline }).call('list_exercises')).toMatchObject({ status: 'error' })
  })
})
