...
- **Deploy:** `bun run deploy` (from `codebase/`, or `apps/web`) = `astro build` + `wrangler deploy`. Manual/owner-triggered — no CI pipeline (deliberate for this slice). Requires a wrangler login (`bunx wrangler login`); `wrangler` is pinned as an `apps/web` devDependency.
- **Local Workers-runtime preview:** `bun run build && bun run preview` — with the Cloudflare adapter, `astro preview` serves the built worker in workerd, so Workers-specific breakage is catchable before deploying.
- **Live URL:** _pending first publish (blocked on owner `wrangler login`; see TASK-024 Log)._
- **Bindings:** the adapter auto-injects `ASSETS` (static files), `IMAGES`, and a `SESSION` KV namespace (Astro's session support — the app does not use sessions; the adapter offers no off switch, so the binding ships unused).
