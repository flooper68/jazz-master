...
## Local database development (TASK-028/TASK-056)
...
`createDatabaseSmokeClient` builds a Drizzle/`pg` connection only when
`DATABASE_URL` is present; otherwise the tRPC context exposes no DB client. The
root `dbSmoke` procedure returns `unconfigured` without throwing when no
database is configured, returns `ok` after a Drizzle `select 1` succeeds, and
returns a generic error shape if the query fails. This proves the server request
path can reach Postgres without making Docker or a live database part of
