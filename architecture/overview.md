...
| Server API | `codebase/apps/web/src/server/trpc/` | tRPC procedures (`init.ts`, `context.ts`, `router.ts`, `routers/`), Zod at every procedure boundary, served through the fetch adapter in `src/pages/trpc/[trpc].ts`. Runs in workerd — no React/DOM. Client consumes it only via `src/app/trpc.ts` (`AppRouter` imported as type only, so server code never enters the client bundle). Context carries `{ auth: { userId } }` from Clerk locals; `protectedProcedure` rejects unauthenticated calls with `UNAUTHORIZED`. Current procedures are public `health`, public server-only `dbSmoke`, public mock practice-data write/read probe, and protected `auth.me` as the foundation probe. ADR-012 makes this the target authorization and app-data boundary: browser code talks to tRPC, not Postgres or localStorage. |
...
reads `locals.auth().userId` from Astro and protected procedures reject missing
identity with `UNAUTHORIZED`.
...
to Clerk/Postgres. The remaining target sequence is Clerk-keyed user anchor,
profile/onboarding, sessions/grades/scores, server-computed daily plans,
