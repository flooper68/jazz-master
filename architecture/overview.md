...
app-hosted Clerk `/sign-in` and `/sign-up` pages, TASK-065 has landed the
Clerk-keyed `users` table anchor, and TASK-066 has moved practice profile and
onboarding completion into Postgres. Sessions, scores, planner snapshots, and
preferences still live in typed localStorage stores until TASK-067 through
TASK-072 land.

Since TASK-021 the app is hosted as a hybrid (ADR-006, accepted 2026-07-06 — EPIC-013): **Astro owns the shell** — the landing page at `/` and any future public/server routes in `apps/web/src/pages/` — and the **entire React practice app is one client-only SPA island under `/app/*`**, mounted by the server-rendered catch-all `src/pages/app/[...path].astro` with `client:only="react"` (no SSR of practice routes; deep links work because the catch-all answers any `/app/...` URL). Build targets Cloudflare Workers via `@astrojs/cloudflare` with `output: 'server'`; deployment itself is TASK-024. Inside the island, routing is TanStack Router (file-based, TASK-022). Since TASK-063, `@clerk/astro` middleware requires a signed-in Clerk user for `/app/*` while `/` stays public, and the React shell renders Clerk's `UserButton` account control. Since TASK-074, signed-out `/app/*` requests redirect to app-hosted `/sign-in` with a `redirect_url` back to the requested app path; `/sign-in` and `/sign-up` render Clerk's prebuilt Astro UI and cross-link locally so password recovery, MFA/2FA, and required session tasks stay inside the app-hosted Clerk flow. Since TASK-023 there is also a **typed tRPC API surface**: server procedures under `apps/web/src/server/trpc/` served by the Astro catch-all endpoint `src/pages/trpc/[trpc].ts` (tRPC fetch adapter), consumed in the island through `@trpc/tanstack-react-query` on one shared React Query `QueryClient` (`src/app/providers.tsx`). tRPC context carries `auth.clerkUserId` for `protectedProcedure`; current procedures are public `health`, public observability probe `dbSmoke`, protected `auth.me`, protected `users.ensure`, and protected `profile.get`/`profile.save`. Since TASK-028/TASK-066, local server-persistence development has a repo-owned Docker Compose Postgres service, `.env.example` connection convention, Drizzle ORM migration workflow, a minimal `users` anchor keyed by Clerk user ID, and normalized `practice_profiles` / `practice_profile_goal_areas` tables for the real profile path. The old smoke-only `mockPractice` procedure was removed in TASK-066 once the profile path verified Clerk/Postgres access; `dbSmoke` remains as the public deployed database observability probe restored by ISSUE-008. Sessions, scores, planner snapshots, and preferences remain local migration state; production database infrastructure remains owner-owned.

```mermaid
flowchart TD
    astro["Astro shell — src/pages/*.astro: landing at /, catch-all app/[...path].astro, tRPC endpoint trpc/[trpc].ts"]
    server["apps/web/src/server/trpc/ — appRouter: typed procedures (public health; protected auth.me/users.ensure/profile.get/profile.save)"]
    ...
    db[(apps/web/src/server/db/ — Drizzle clients + users/profile repositories; target app-data source of truth)]
    astro --> shell
    astro --> server
    server -. authenticated user context .-> clerk
    server -. protected users/profile repositories .-> db
...
| Auth | `codebase/apps/web/src/pages/sign-in.astro`, `codebase/apps/web/src/pages/sign-up.astro`, `codebase/apps/web/src/middleware.ts` + `codebase/apps/web/src/server/auth/` | `@clerk/astro` middleware protects `/app/*` and leaves `/` public. Runtime startup requests fail clearly when `PUBLIC_CLERK_PUBLISHABLE_KEY` or `CLERK_SECRET_KEY` is missing, so local dev uses real Clerk keys rather than Clerk keyless mode. App-hosted Clerk pages at `/sign-in` and `/sign-up` render the prebuilt auth UI, cross-link locally, and fall back to `/app` when no `redirect_url` is present. |
| Server API | `codebase/apps/web/src/server/trpc/` | tRPC procedures (`init.ts`, `context.ts`, `router.ts`, `routers/`), Zod at every procedure boundary, served through the fetch adapter in `src/pages/trpc/[trpc].ts`. Runs in workerd — no React/DOM. Client consumes it only via `src/app/trpc.ts` (`AppRouter` imported as type only, so server code never enters the client bundle). Context carries `{ auth: { clerkUserId } }` from Clerk locals; `protectedProcedure` rejects unauthenticated calls with `UNAUTHORIZED`. Current procedures are public `health`, public observability probe `dbSmoke`, protected `auth.me`, protected `users.ensure` for the DB-backed app-user anchor, and protected `profile.get`/`profile.save` for practice profile/onboarding data. The tRPC endpoint emits safe structured Worker logs through `src/server/observability/logger.ts`: `trpc.request.completed` for request outcomes and `db.smoke.completed` for Hyperdrive/Postgres smoke status. Logs carry event/outcome/procedure/status/duration/request metadata and explicitly omit or redact Clerk identifiers, cookies, auth headers, database URLs, request bodies, response bodies, and SQL. ADR-012 makes this the target authorization and app-data boundary: browser code talks to tRPC, not Postgres or localStorage. |
| Server database | `codebase/apps/web/src/server/db/` + `codebase/apps/web/drizzle.config.ts` + `codebase/apps/migration/drizzle/` | Server-only Drizzle schema, generated SQL migrations, a minimal `users` table keyed by `clerk_user_id`, and normalized profile tables: `practice_profiles` for levels/minutes/onboarding completion and `practice_profile_goal_areas` for ordered goal priorities. `ensureUser(clerkUserId)` creates or reuses the user row; `profiles.ts` reads/writes profile data for that Clerk user. Clerk-owned email/name/profile data stays in Clerk. `drizzle-orm`/`pg` stay out of React, components, and `packages/theory`; no runtime request path applies migrations. ADR-012 makes Postgres the target source of truth for long-run app data, scoped by Clerk user ID, while sessions/scores/planner/preferences remain local until their migration tasks remove those stores. |
...
procedures reject missing identity with `UNAUTHORIZED`.
...
migrated. The remaining target sequence is sessions/grades/scores,
server-computed daily plans, preferences, backup/import removal, localStorage
layer removal, then regression.
...
`apps/web/src/storage/` exposes `defineStore<T>({ name, version, defaultValue, migrate? })`, returning a typed `Store<T>` (`get`/`set`/`update`/`reset`). Values persist under `jazz-master:<name>` in a `{ version, data }` envelope. Reads never throw: missing keys, corrupt JSON, malformed envelopes, versions from the future, and failed migrations all fall back to `defaultValue()` with a `console.warn`. When the persisted version is older, `migrate(persisted, fromVersion)` upgrades the data and the result is written back. Current durable local user-data stores are `sessions`, `daily-plans`, `play-along-tempos` (per-exercise slow-practice BPM), `notation-preferences`, and `scoring-preferences` (lenient/standard/strict take-scoring tolerance); profile/onboarding has moved to Clerk/Postgres. Session `durationSeconds` is accumulated active exercise time: the runner starts counting when playback/recording begins for an exercise and stops when that playthrough completes, so setup and grading prompt time are excluded. `PracticeSession.results[]` can now carry machine-score metadata per exercise: score, tolerance preset, pitch/timing/completeness components, per-note verdicts, and detected-extra count; session-level `score` is the mean of scored exercise results for history/dashboard summaries. RES-014/INS-023 add an important caveat: Safari/WebKit can evict script-writable storage after a period without user interaction. Since TASK-066, the Profile page backup export/import covers plans, history, and practice settings but no longer carries profile data. The import contract lives in `storage/backup.ts`: it rejects malformed, oversized, unsupported-version, or bad-date backups, then writes versioned envelopes transactionally and verifies durable bytes before reporting success. **Convention: no direct `localStorage` access outside `src/storage/`** — remaining local features still define stores while they await migration. Extraction to `packages/storage` waits for a second consumer (ADR-005); React hooks over stores are deferred to first use.
