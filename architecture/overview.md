...
- **Credential boundary (ADR-009):** agents never hold deploy credentials — no `wrangler login` on development machines, ever. The only deploy credential is a Cloudflare API token scoped to Workers Scripts, stored as the `CLOUDFLARE_API_TOKEN` GitHub Actions secret (with `CLOUDFLARE_ACCOUNT_ID` beside it), created and rotated by the owner.
- **Dev deploys (CI):** `.github/workflows/deploy-dev.yml` runs on every push to `main` (plus `workflow_dispatch` for re-runs): `bun install --frozen-lockfile` → `bun run check` → `bun run deploy` (= `astro build` + `wrangler deploy`). The deployed `jazz-master-web` worker **is the dev environment**; a red `deploy-dev` run is the signal that dev is stale.
- **Production:** does not exist yet — gated TASK-036 (separate environment, manual UI trigger, enforcement question open).
- **Local Workers-runtime preview:** `bun run build && bun run preview` — with the Cloudflare adapter, `astro preview` serves the built worker in workerd, so Workers-specific breakage is catchable before pushing.
- **Live dev URL:** _pending the first green CI deploy (waiting on the owner-created repo secrets; see TASK-024 Log)._
