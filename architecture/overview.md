...
flowchart TD
    ...
    subgraph SPA island — client:only under /app/*
        ...
        storage[(apps/web/src/storage/ — temporary typed localStorage preferences until migrated)]
    end
    db[(apps/web/src/server/db/ — Drizzle clients + users/profile/session repositories; target app-data source of truth)]
    astro --> shell
    astro --> server
    server -. authenticated user context .-> clerk
    server -. protected users/profile/session repositories .-> db
    ...
    audio --> theory
    scoring --> theory
    pages -. temporary preference persistence before migration .-> storage
```
...
| Persistence | `codebase/apps/web/src/storage/` now; `codebase/apps/web/src/appData/` target for shared contracts | Current typed stores over localStorage via `defineStore` remain as temporary migration state for preferences only. Profile, sessions/scores, and daily planning have moved off localStorage. **No direct `localStorage` access outside `src/storage/` while it exists.** ADR-012 supersedes localStorage as the target: each feature migration removes its store path, server code owns Drizzle/Postgres persistence, and client-safe app-data contracts move to `src/appData/` or equivalent before `src/storage/` is deleted. |
...
curriculum. The remaining target sequence is preferences, backup/import
removal, localStorage layer removal, then regression.
...
`apps/web/src/storage/` exposes `defineStore<T>({ name, version, defaultValue, migrate? })`, returning a typed `Store<T>` (`get`/`set`/`update`/`reset`). Values persist under `jazz-master:<name>` in a `{ version, data }` envelope. Reads never throw: missing keys, corrupt JSON, malformed envelopes, versions from the future, and failed migrations all fall back to `defaultValue()` with a `console.warn`. When the persisted version is older, `migrate(persisted, fromVersion)` upgrades the data and the result is written back. Current local stores are now preferences only: `play-along-tempos` (per-exercise slow-practice BPM), `notation-preferences`, and `scoring-preferences` (lenient/standard/strict take-scoring tolerance). Profile/onboarding, sessions/scores, and daily planning have moved to Clerk/Postgres/tRPC. `planner.today` computes today's plan on demand from the authenticated profile, persisted session history, curriculum lessons, and the browser-local plan date; legacy `jazz-master:daily-plans` data is ignored. RES-014/INS-023 add an important caveat for remaining preference stores: Safari/WebKit can evict script-writable storage after a period without user interaction. Since TASK-068, the Profile page backup export/import covers only remaining local practice settings and ignores legacy plan/profile/session data. The import contract lives in `storage/backup.ts`: it rejects malformed, oversized, unsupported-version, or bad-date preference backups, then writes versioned envelopes transactionally and verifies durable bytes before reporting success. **Convention: no direct `localStorage` access outside `src/storage/`** — remaining local features still define stores while they await migration. Extraction to `packages/storage` waits for a second consumer (ADR-005); React hooks over stores are deferred to first use.
