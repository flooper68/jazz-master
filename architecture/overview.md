...
flowchart TD
    ...
    db[(apps/web/src/server/db/ — Drizzle clients + Postgres smoke/mock/user-anchor paths; target app-data source of truth)]
    astro --> shell
    astro --> server
    server -. authenticated user context .-> clerk
    server -. server-only dbSmoke/mockPractice/users .-> db
...
to Clerk/Postgres. The remaining target sequence is profile/onboarding,
sessions/grades/scores, server-computed daily plans, preferences, backup/import
