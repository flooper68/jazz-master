...
migrated. TASK-063 has landed the Clerk foundation, TASK-065 has landed the
...
| Auth | `codebase/apps/web/src/middleware.ts` + `codebase/apps/web/src/server/auth/` | `@clerk/astro` middleware protects `/app/*` and leaves `/` public. Runtime startup requests fail clearly when `PUBLIC_CLERK_PUBLISHABLE_KEY` or `CLERK_SECRET_KEY` is missing, so local dev uses real Clerk keys rather than Clerk keyless mode. |
...
`/app/*` redirects signed-out users to Clerk sign-in, `/` remains public, and
the app shell exposes Clerk's `UserButton` for account control. tRPC context
reads `locals.auth().userId` from Astro into `auth.clerkUserId`, and protected
