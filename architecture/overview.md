...
Jazz Master has completed the implementation phase of its persistence migration.
ADR-012 supersedes the original local-first browser state model (ADR-002): Clerk
owns identity, `/app/*` requires sign-in, browser code talks to typed tRPC
procedures, server code owns Drizzle/Postgres access, and Postgres becomes the
source of truth for long-run app data. Existing local browser data is not
...
TASK-070 removed local JSON backup/import, and TASK-071 removed the generic
browser-persistence layer after shared contracts moved to `src/appData/`.
TASK-072 completed the end-to-end regression pass, including a real Clerk
session for the dedicated test account plus protected Postgres reads/writes, so
ADR-012's migration and EPIC-013 are complete.
...
flowchart TD
    ...
    appData[apps/web/src/appData/ — client-safe profile, session, and preference contracts]
...
| Auth | `codebase/apps/web/src/pages/sign-in*`, `codebase/apps/web/src/pages/sign-up*`, `codebase/apps/web/src/middleware.ts` + `codebase/apps/web/src/server/auth/` | `@clerk/astro` middleware protects `/app/*` and leaves `/` public. Runtime startup requests fail clearly when `PUBLIC_CLERK_PUBLISHABLE_KEY` or `CLERK_SECRET_KEY` is missing, so local dev uses real Clerk keys rather than Clerk keyless mode. Exact and rest-parameter app-hosted Clerk pages render the prebuilt auth UI inside the public shell, so `/sign-in`, `/sign-up`, and nested factor/verification/session-task states stay routable; they cross-link locally, emit `noindex`, and fall back to `/app` when no `redirect_url` is present. |
...
| Persistence | `codebase/apps/web/src/appData/` + `src/server/db/` | Client-safe profile/session/preference contracts live in `appData`; durable feature data is Clerk-scoped through tRPC/Drizzle/Postgres. Browser product code has no local persistence layer, and legacy browser payloads are ignored. |
...
`preferences.ts`; local JSON backup/import and the generic browser-persistence
layer are gone. TASK-072's passing browser and Postgres regression—including a
real Clerk identity handoff and a clear-storage synthetic-auth durability
check—closes the migration.
...
- **Local Workers-runtime preview:** `bun run build && bun run preview` — with the Cloudflare adapter, `astro preview` serves the built worker in workerd, so Workers-specific breakage is catchable before pushing. Clerk secrets reach it through the process environment, not a dotenv file (recipe in `apps/web/README.md`). There is no `deploy` script: TASK-086 removed it once ADR-009's 2026-08-02 amendment made a local publish actually possible, because a locally built bundle can carry inlined secrets.
- **Build output carries no secrets (TASK-086):** Astro loads `.env` in every mode and `@clerk/astro` reads `import.meta.env` dynamically, so any value in `.env`/`.dev.vars` becomes a literal in `dist/server/`. Local secrets therefore live in `apps/web/.env.development` (dev-mode only), and `apps/web/scripts/assertCleanBuildOutput.ts` fails the build if a secret-shaped literal or a `.dev.vars` appears under `dist/`. Workers Builds has no dotenv file at all; the deployed Worker gets `CLERK_SECRET_KEY` from its owner-managed secret binding.
...
override exists. Existing browser preference keys are ignored.
TASK-071 removed the obsolete generic typed browser-persistence wrapper and its
compatibility exports. Shared profile, session, and preference contracts now
live entirely in `apps/web/src/appData/`.

ADR-012 changed the destination. The original typed stores are migration
history, not the current product architecture. The implemented architecture is
Clerk-authenticated tRPC over server-owned Drizzle/Postgres data, with existing
local browser data discarded instead of migrated. Feature migrations removed
their browser persistence paths; backup/import and `src/storage/` are now gone.
