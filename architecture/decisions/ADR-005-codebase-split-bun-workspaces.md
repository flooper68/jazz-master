...
4. **Root shim.** ~~A root `package.json` contains delegating scripts only (`"check": "bun --cwd codebase run check"`, same for `dev`/`test`) and no dependencies. `bun run check` from the repo root remains THE gate, so every process doc and agent habit keeps working verbatim.~~ **Superseded during TASK-027 (owner instruction, 2026-07-05): there is no root `package.json` at all.** The repo root holds knowledge and dotfiles only; commands run inside `codebase/`, or from the root as `bun run --cwd codebase <script>`. `bun run check` (in `codebase/`) remains THE gate; CLAUDE.md/AGENTS.md document the invocation.
5. **Gate wiring.** `check` keeps its meaning (typecheck + lint + test + build): `tsc -b` with project references from `codebase/` (shared `tsconfig.base.json`, `composite: true` packages), oxlint over `codebase/`, Vitest `projects` covering all workspaces, build via `bun run --filter './apps/*' build` (Bun path filters need the `./` prefix).
...
- **No root shim (always `cd codebase`)** — originally rejected (agent friction), then **chosen by the owner during TASK-027**: a root `package.json` misrepresents the root as a code directory. Decision point 4 above is amended accordingly.
